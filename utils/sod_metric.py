...
def _prepare_data(pred: np.ndarray, gt: np.ndarray, thresh=128) -> tuple:
    """
...
class dice_iou_tool(object):
    ...
    def step(self, pred, gt):
        pred_binary = np.zeros(pred.shape).astype(np.uint8)
        pred_binary[pred>self.thresh] = 1
        self.dice_list.append(med_metric.binary.dc(pred_binary, gt))
        self.iou_list.append(med_metric.binary.jc(pred_binary, gt))

    def get_results(self):
        mean_dice = np.mean(np.array(self.dice_list))
        mean_iou = np.mean(np.array(self.iou_list))
